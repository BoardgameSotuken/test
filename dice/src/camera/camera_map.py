...
class Main:
                ...
                if ids is not None:
                    if start_goal_check:
                        ...
                        if cv2.waitKey(1) & 0xFF == ord('p'):
                            map_id_list = self.make_map(markers_id_pos) 
                            self.data_sending(map_id_list)
    ...
    def make_map(self, marker_id_point):
        ...
        marker_id_pos = np.delete(marker_id_pos, index_28, axis=0)  #配列から削除
        ...
        while len(marker_id_pos) > 0:  #まだマーカーが残っている限り繰り返す
            ...
            marker_id_pos = np.delete(marker_id_pos, next_marker_index, axis=0)
    ...
    def data_sending(self, marker_id):
        ...
        try:
            ...
            # データを送信するためのペイロードを作成
            data = {'tag' : 'map', 'marker_id': marker_id}
