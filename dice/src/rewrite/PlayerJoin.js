...
const playerNumList = Array(4).fill(null);
...
function sendPlayerNum() {
	for (let i = 0; i < 4; i++) {
		...
		if (ws) {
			...
			ws.send(JSON.stringify({ tag: 'four', data: 4, message: message , playerNum: playerNum}));
		}
	}
}
...
function closeWs() {
    allWsList.forEach((ws, index) => {
        if (ws && !playerList.includes(ws.uuid)) {
            if (ws.readyState === ws.OPEN) {  // 接続が生きている場合のみ閉じる
                ws.close();
...
export default { setUUID, setPlayerList, deleteUUID, sendPlayerCnt, sendPlayerNum, getPlayerList, getWsList };